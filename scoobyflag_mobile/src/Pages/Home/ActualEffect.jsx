    ...
    return (
        <ScrollView horizontal={true} style={{position:"absolute",paddingHorizontal:"5%", bottom:125,zIndex:2,flexDirection:"row"}}>
            {actualEffects.map((effect,index) => {
                if(effect.endAt.valueOf() <= now.valueOf()){
                    setActualEffects(cur => cur.filter(e => e.id!==effect.id));
                }
                const timeLeft=effect.endAt.diff(now,["seconds"]).toObject();
                return (
                    <View
                    key={index}
                    style={{
                    ...
                    <Text style={{color: COLORS.white, paddingHorizontal: 2}}>{`${timeLeft.seconds.toFixed(0)}s`}</Text>
                  </View>
            )})}
        </ScrollView>
    )
